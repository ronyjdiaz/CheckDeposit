...
public class ResideMenu extends FrameLayout {
    ...
    public ResideMenu(Context context) {
        ...
        super(context);
        Log.v(this.getClass().getName(),"ResideMenu - " + "Iniciando");
        initViews(context);
        Log.v(this.getClass().getName(),"ResideMenu - " + "Salida");
    }

    private void initViews(Context context) {
        Log.v(this.getClass().getName(),"initViews - " + "Iniciando");
        ...
        nombreUser = (TextView) findViewById(R.id.tv_nombre_user_rm);
        //avatar = (ImageView)findViewById(R.id.iv_profile_user_avatar);


        Log.v(this.getClass().getName(),"initViews - " + "Salida");
     ...
     */
    @Deprecated
    public void addMenuItem(ResideMenuItem menuItem, boolean asSubmenu, int subMenuPosition) {
     ...
     * @param direction
     */
    public void addMenuItem(ResideMenuItem menuItem, int direction, boolean asSubmenu, int subMenuPosition) {

        if (direction == DIRECTION_LEFT) {
            // layoutRightMenu.setLayoutTransition(new LayoutTransition());
            if (asSubmenu) {
                ArrayList<ResideMenuItem> tempArray = new ArrayList<ResideMenuItem>();
                for (int i = 0; i <= subMenuPosition; i++) {
                    tempArray.add(this.leftMenuItems.get(i));
                }
                tempArray.add(menuItem);
                for (int j = subMenuPosition + 1; j < this.leftMenuItems.size(); j++) {
                    tempArray.add(this.leftMenuItems.get(j));
                }
                this.leftMenuItems = tempArray;
                //rebuildMenu();
                layoutLeftMenu.addView(menuItem, subMenuPosition + 1);
            } else {
            ...
            if (asSubmenu) {
                ArrayList<ResideMenuItem> tempArray = new ArrayList<ResideMenuItem>();
                for (int i = 0; i <= subMenuPosition; i++) {
                    tempArray.add(this.rightMenuItems.get(i));
                }
                tempArray.add(menuItem);
                for (int j = subMenuPosition + 1; j < this.rightMenuItems.size(); j++) {
                    tempArray.add(this.rightMenuItems.get(j));
                }
                this.rightMenuItems = tempArray;
                //rebuildMenu();
                if (menuItem.getParent() != null) {
                    layoutRightMenu.addView(menuItem); //((ViewGroup) menuItem.getParent()).removeView(menuItem);
                }
                layoutRightMenu.addView(menuItem, subMenuPosition + 1);
            } else {
                /*if (menuItem.getParent() != null)
                    ((ViewGroup) menuItem.getParent()).removeView(menuItem);*/
                if (menuItem.getParent() != null) {
                ...
                rightMenuItems.remove(menuItem);
    ...
    public void closeSubmenuRight_byName(ResideMenuItem rmit) {
        String title = rmit.getTv_title().getText().toString();
        for (ResideMenuItem rmi : rightMenuItems) {
            if (rmi.getTv_title().getText().toString().equalsIgnoreCase(title)) {
                if (rmi.getParent() != null) {
                    //((ViewGroup) rmi.getParent()).removeView(rmi);
                    layoutRightMenu.removeView(rmi);
                }
                //layoutRightMenu.removeView(rmi);
                break;
            }
    ...
    public void closeSubmenuLeft(int position, int options) {

        //layoutRightMenu.setLayoutTransition(null);
        for (int i = position; i < options; i++) {
            // this.rightMenuItems.remove(position+1);
            layoutLeftMenu.removeView(this.leftMenuItems.get(i + 1));
    ...
    private boolean isInIgnoredView(MotionEvent ev) {
        ...
        for (View v : ignoredViews) {
            ...
            float parentWidth = metrics.widthPixels;
            if (rect.contains((int) ev.getX(), (int) ev.getY()) && !(lastActionDownX > parentWidth - (parentWidth * 0.10)) && !(lastActionDownX < parentWidth * 0.10))
                return true;
        }
        return false;
    }
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (pressedState == PRESSED_DOWN) {
                    ...
                    DisplayMetrics metrics = parentActivity.getResources().getDisplayMetrics();
                    float parentWidth = metrics.widthPixels;
                    if (isOpened() || (!isOpened() && lastActionDownX > parentWidth - (parentWidth * 0.10) || lastActionDownX < parentWidth * 0.10)) {
    ...
    public void closeSubmenuRight(int position, int options) {
        //layoutRightMenu.setLayoutTransition(null);
        for (int i = position; i < options; i++) {
            // this.rightMenuItems.remove(position+1);

            ResideMenuItem rmi = this.rightMenuItems.get(i + 1);
            //layoutRightMenu.removeView(rmi);
            ((ViewGroup) rmi.getParent()).removeView(rmi);
